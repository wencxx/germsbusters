    ...
    <div class="flex justify-center">
        <div class="mt-14 p-10 bg-white w-full max-w-[80%] h-fit min-h-96 rounded-md border space-y-3">
            <div class="flex justify-between">
                <h1 class="text-gray-500 font-semibold tracking-wide text-lg">Pending Reservations</h1>
...
const filteredReservations = computed(() => {
    if (!searchFilter.value) return pendingReservations.value;
...
const accepted = () => {
    ...
    const index = pendingReservations.value.indexOf(reservationData.value)
...
const rejected = async (data) => {
    try {
        ...
        pendingReservations.value.splice(reservationIndexToReject.value, 1)
