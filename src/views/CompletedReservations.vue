    ...
    <div class="flex justify-center">
        <div class="mt-14 p-10 bg-white w-full max-w-[80%] h-fit min-h-96 rounded-md border space-y-3">
            <div class="flex justify-between">
                <h1 class="text-gray-500 font-semibold tracking-wide text-lg">Completed Reservations</h1>
...
const filteredReservations = computed(() => {
    if (!searchFilter.value) return completedReservations.value;

    const searchTerm = searchFilter.value.toLowerCase();

    return completedReservations.value
        .filter(reservation => 
...
const accepted = () => {
    ...
    const index = completedReservations.value.indexOf(reservationData.value)
...
const rejected = async (data) => {
    try {
        ...
        completedReservations.value.splice(reservationIndexToReject.value, 1)
